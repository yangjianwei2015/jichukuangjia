...
//  CGFloat y = ceilf(0.1 * tabFrame.size.height);
  if (badgeValue < 10) {
    [self initUnreadCountButton:CGRectMake(x, 0, 18, 18) tag:888+index badgeValue:badgeValue];
  }
  if (badgeValue >= 10 && badgeValue < 100 ) {
    [self initUnreadCountButton:CGRectMake(x, 0, 22, 18) tag:888+index badgeValue:badgeValue];
  }
  if (badgeValue >= 100) {
    RCDTabBarBtn *btn = [[RCDTabBarBtn alloc] initWithFrame:CGRectMake(x, 0, 22, 18)];
